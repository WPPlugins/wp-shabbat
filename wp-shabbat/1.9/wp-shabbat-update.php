...
if ( ($options['nextUpdate'] <  current_time('timestamp')) || ( $options['lastUpdate'] == '0' ) ){
	...
	if ( ($options['lastUpdate'] == '0' ) ){ 
			...
			if (get_locale() == 'he_IL'){ 
				$locale = '<div style="direction: rtl;font-size:30px;">';
			}
			else {
				$locale = '<div style="font-size:30px;">';
			}
			...
			<br/>'.__('and thank you for using WP-Shabbat Plugin this plugin is','WP-Shabbat').' <strong> '.__('free so please support us','WP-Shabbat').'</strong> <a href="'.$adminUrl.'">'.__('By helping us with the cost of the plugin','WP-Shabbat').' </a>
